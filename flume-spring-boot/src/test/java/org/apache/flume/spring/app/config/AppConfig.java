...
public class AppConfig extends AbstractFlumeConfiguration {
    ...
    @Bean
    public SourceRunner seqSource(
            Channel memoryChannel, @Qualifier("source1Properties") Map<String, String> source1Properties) {
        ChannelSelector selector = new ReplicatingChannelSelector();
        selector.setChannels(listOf(memoryChannel));
        SourceRunner runner = configureSource("source1", SequenceGeneratorSource.class, selector, source1Properties);
    ...
    public SinkRunner nullSink(Channel memoryChannel) {
        ...
        return createSinkRunner(configureSinkProcessor(null, DefaultSinkProcessor.class, listOf(sink)));
    }
}
